...
namespace webserver::sock {
        ...
        host_(host)
    {
        ...
        if (socket_ < 0) {
            ...
            throw SocketException{ "Create socket failed.", socket_ };
        }
	}
	...
	void LinuxSocketTcp::ConnectTimeout(const std::string& host, uint32_t port, uint32_t timeout_ms) {
        ...
        if( (arg = fcntl(socket_, F_GETFL , NULL)) < 0) { 
            ...
            Close();
            throw SocketException{ "Error fcntl F_GETFL", errno };
        }
        arg |= O_NONBLOCK; 
        if( fcntl(sock, F_SETFL, arg) < 0) { 
            lLog(lError) << "Error fcntl(..., F_SETFL) " <<  strerror(errno)); 
            Close();
            throw SocketException{ "Error fcntl F_SETFL", errno };
        }

        ret = getaddrinfo(host, portnumber_string, &hints, &result);
        if (ret != 0) {
            lLog(lError) << "ConnectTimeoutTcp error: DNS failure with " << host << " " << ret " " << strerror(errno));
            freeaddrinfo(result);
            Close();
            throw SocketException{ "ConnectTimeoutTcp error: DNS failure with", errno };
        }
        ...
        for (rp = result; rp != NULL; rp = rp->ai_next) {
            ...
            if(errno == EINPROGRESS) {
                do {
                    FD_ZERO(&myset);
                    FD_SET(sock, &myset);   // socket zur Menge der beschreibbaren Deskriptoren hinzufügen
                    ret = select(sock + 1, NULL, &myset, NULL, (timeout_ms > 0 ? &tv : NULL));// select ist auf 1024 deskriptoren beschränkt, bei mehr = undefined behaviour
                    if(ret < 0 && errno != EINTR) {
                        lLog(lError) << "ConnectTimeoutTcp error: in select " << errno << " " << strerror(errno);
                        freeaddrinfo(result);
                        Close();
                        throw SocketException{ "ConnectTimeoutTcp error: in select", errno };
                    }else if(ret > 0) {
                        lon = sizeof(int);
                        ...
                        if(getsockopt(sock, SOL_SOCKET, SO_ERROR, (void*)(&valopt), &lon) < 0) {    
                            ...
                            freeaddrinfo(result);
                            Close();
                            throw SocketException{ "ConnectTimeoutTcp error: in getsockopt()", errno };
                        }
                        // check the value returned
                        if(valopt) {
                            LOG_message("ConnectTimeoutTcp error: in delay connect()" <<  valopt << " " << strerror(valopt));
                            freeaddrinfo(result);
                            Close();
                            throw SocketException{ "ConnectTimeoutTcp error: in delay connect()", valopt};
                        }

                        // Set to blocking mode again... 
                        if( (arg = fcntl(sock, F_GETFL, NULL)) < 0) { 
                            LOG_message("ConnectTimeoutTcp error: fcntl(..., F_GETFL) " << errno << " " << strerror(errno)); 
                            freeaddrinfo(result);
                            Close();
                            throw SocketException{ "ConnectTimeoutTcp error: fcntl(..., F_GETFL)", errno};
                        } 
                        arg &= (~O_NONBLOCK); 
                        if( fcntl(sock, F_SETFL, arg) < 0) { 
                            ...
                            freeaddrinfo(result);
                            Close();
                            throw SocketException{ "ConnectTimeoutTcp error: fcntl(..., F_SETFL)", errno};
                        } 
    ...
    Result<int32_t> LinuxSocketTcp::Send(const std::vector<char>& data) const {
        ...
        if((sent = send(socket_, data.data(), data.size(), 0)) < 0) {
            lLog(lError) << "SOCK_SendTcp error: " << errno << " " << strerror(errno));
            return Result<int32_t>{strerror(errno)), errno};
        }
    ...
    Result<std::vector<char>> LinuxSocketTcp::Receive() const {
        ...
        if((received = recv(socket_, vec.data(), receive_size, 0)) < 0) {
            ...
            return Result<std::vector<char>>{strerror(errno), std::vector<char>{}};
        }
        vec.resize(received);
